...
class ListEndpointGroups(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            metavar="<description>",
            help=_(
                "Provides supplementary information about the Direct "
                "Connect Endpoint Group."
            ),
        )
        parser.add_argument(
            "--project_id", metavar="<project_id>", help=_("Specifies the project ID.")
        )
        parser.add_argument(
            "--type",
            metavar="<type>",
        ...
        parser.add_argument(
            ...
            default=[],
            help=_(
                "Specifies the list of the endpoints in a Direct Connect "
                "Endpoint Group."
            ),
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        args_list = ["id", "name", "description", "project_id", "type"]
...
class CreateEndpointGroup(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        args_list = ["name", "description", "project_id", "type"]
        attrs = {}
        endpoints = []
        if parsed_args.endpoints:
            for endpoint in parsed_args.endpoints:
...
class UpdateEndpointGroup(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.endpoint_group:
            endpoint_group = client.find_endpoint_group(parsed_args.endpoint_group)
            obj = client.update_endpoint_group(endpoint_group.id, **attrs)
