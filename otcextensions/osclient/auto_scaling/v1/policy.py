...
class ShowAutoScalingPolicy(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            metavar="<policy>",
            help=_(
                "ID of the configuration policy\n"
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.group:
            ...
            obj = client.find_policy(
                parsed_args.policy, group=group, ignore_missing=False
            )
        else:
        ...
        data = utils.get_dict_properties(fmt, self.columns, formatters={})
...
class CreateAutoScalingPolicy(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument("name", metavar="<name>", help=_("AS Policy name"))
        parser.add_argument(
            "--group",
            metavar="<group>",
            required=True,
        ...
        parser.add_argument(
            ...
            help=_("Specifies the policy operation " "Can be [`ADD`, `REMOVE`, `SET`]"),
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            metavar="<start_time>",
            help=_(
                "Specifies the start time in of the action in the "
        ...
        parser.add_argument(
            ...
            metavar="<end_time>",
            help=_(
                "Specifies the end time in of the action in the "
                "`YYYY-MM-DDThh:mmZ` format\n"
    ...
    def take_action(self, parsed_args):
        ...
        if policy_type not in self.POLICY_TYPES:
            msg = _("Unsupported policy type. Should be one of %s") % self.POLICY_TYPES
        ...
        if parsed_args.action_instance_number:
            policy_action["instance_number"] = parsed_args.action_instance_number
...
class DeleteAutoScalingPolicy(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "policy", nargs="+", metavar="<policy>", help=_("AS Policy ID")
        )
        return parser
...
class UpdateAutoScalingPolicy(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument("policy", metavar="<policy>", help=_("AS Policy ID"))
        parser.add_argument(
            "--group",
            metavar="<group>",
            required=True,
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.type:
            ...
            if policy_type not in self.POLICY_TYPES:
                msg = (
                    _("Unsupported policy type. Should be one of %s")
                    % self.POLICY_TYPES
                )
                raise argparse.ArgumentTypeError(msg)
            else:
        ...
        policy = client.update_policy(policy, **policy_attrs)
...
class ExecuteAutoScalingPolicy(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "policy", metavar="<policy>", help=_("AS Policy ID or name")
        )
        return parser
