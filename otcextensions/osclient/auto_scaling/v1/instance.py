...
class ListAutoScalingInstance(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            help=_(
                "Life cycle state of the instances to query\n"
        ...
        parser.add_argument(
            ...
            help=_(
                "Health status of the instances to query\n"
...
class RemoveAutoScalingInstance(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "instance", metavar="<instance>", help=_("AS Instance ID to be deleted")
        )
        parser.add_argument(
            "--delete-instance",
            action="store_true",
...
class BatchActionAutoScalingInstance(command.Command):
    ...
    def get_parser(self, prog_name):
        parser = super(BatchActionAutoScalingInstance, self).get_parser(prog_name)
        parser.add_argument(
            "--group", metavar="<group>", required=True, help=_("AS Group ID or Name")
        )
        parser.add_argument(
            "action",
            metavar="<action>",
    ...
    def take_action(self, parsed_args):
        ...
        if action not in self.SUPPORTED_ACTIONS:
            msg = _("Action %(action)s is not one of " "the supported %(types)s") % {
