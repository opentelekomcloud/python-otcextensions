...
class CreateAutoScalingConfig(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument("name", metavar="<name>", help=_("AS Configuration name"))
        ...
        parser.add_argument(
            "--key", metavar="<key>", help=_("Key name for the new ECS instance")
        )
        parser.add_argument(
            "--public-ip-bandwith",
            ...
            type=int,
            help=_(
                "Defines EIP Bandwith (Mbit/s) to be attached "
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.instance_id:
            ...
            if not all([parsed_args.flavor, parsed_args.image_id, parsed_args.disk]):
                msg = _(
                    "Either instance_id or all of the "
                    "[flavor, image, disk] "
                    "should be given"
                )
...
class DeleteAutoScalingConfig(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "config", nargs="+", metavar="<config>", help=_("AS Configuration ID")
        )
        return parser
