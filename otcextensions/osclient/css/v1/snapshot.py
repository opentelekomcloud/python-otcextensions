...
FREQUENCY_CHOICES = ["day", "hour", "sun", "mon", "tue", "wed", "thu", "fri", "sat"]
...
class ShowSnapshot(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        data = utils.get_item_properties(snapshot, columns, formatters=_formatters)
...
class DeleteSnapshot(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        for snapshotId in parsed_args.snapshot:
            try:
                client.delete_snapshot(cluster, snapshotId, ignore_missing=False)
