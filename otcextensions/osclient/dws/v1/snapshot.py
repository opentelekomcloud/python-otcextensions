...
class CreateSnapshot(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            'cluster',
            metavar='<cluster>',
            help=_('ID or name of the cluster for which you want to '
                   'create a snapshot.')
        )
        parser.add_argument(
            'name',
        ...
        parser.add_argument(
            '--description',
            metavar='<description>',
            help=_('Snapshot description. If no value is specified, '
...
class ShowSnapshot(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        return client.find_snapshot(parsed_args.snapshot)
...
class RestoreSnapshot(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            help=_('AZ of a cluster. The default value is the same '
        ...
        network_group.add_argument(
            ...
            metavar='<router_id>',
            dest='vpc_id',
            help=_('Router ID, which is used for configuring cluster '
        ...
        network_group.add_argument(
            ...
            metavar='<network_id>',
            dest='subnet_id',
            help=_('Network ID, which is used for configuring cluster '
                   'network. The default value is the same as that of '
                   'the original cluster.')
        )
        network_group.add_argument(
            ...
            help=_('Security group ID, which is used for configuring '
        ...
        parser.add_argument(
            '--floating-ip',
            metavar='<floating_ip>',
            help=_('Public IP address. If the parameter is not specified, '
    ...
    def take_action(self, parsed_args):
        ...
        if floating_ip and floating_ip.lower() == 'auto':
            ...
            resp = network_client.find_ip(floating_ip, ignore_missing=False)
            attrs['public_ip'] = {
...
class DeleteSnapshot(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        for name_or_id in parsed_args.snapshot:
            try:
                snapshot = client.find_snapshot(name_or_id,
