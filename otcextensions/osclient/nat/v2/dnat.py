...
class ListDnatRules(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            metavar="<private_ip>",
            help=_(
                "Specifies the private IP address, for example, "
        ...
        parser.add_argument(
            ...
            help=_(
                "Specifies port used by ECSs or BMSs to provide "
        ...
        parser.add_argument(
            ...
            help=_(
                "Specifies whether the DNAT rule is enabled or "
        ...
        parser.add_argument(
            ...
            metavar="<created_at>",
            help=_(
                "Specifies when the DNAT rule is created (UTC time). "
    ...
    def take_action(self, parsed_args):
        ...
        for dnat in parsed_args.dnat:
            try:
                ...
                result += 1
                LOG.error(
                    _("Failed to delete DNAT rule with " "ID '%(dnat)s': %(e)s"),
                    {"dnat": dnat, "e": e},
                )
        if result > 0:
            total = len(parsed_args.dnat)
            msg = _("%(result)s of %(total)s DNAT Rule(s) failed " "to delete.") % {
                "result": result,
                "total": total,
            }
