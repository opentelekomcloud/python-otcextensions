...
class ListSubscription(command.Lister):
    ...
    columns = ("ID", "Protocol", "Topic URN", "Owner", "Endpoint", "Status")
...
class CreateSubscription(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        args_list = ["protocol", "endpoint", "remark"]
        attrs = {}
        for arg in args_list:
            val = getattr(parsed_args, arg)
            if val:
    ...
    def take_action(self, parsed_args):
        ...
        for subscription in parsed_args.subscription:
            try:
                ...
                LOG.error(
                    _(
                        "Failed to delete SMN Subscription with "
        ...
        if result > 0:
            ...
            msg = _(
                "%(result)s of %(total)s SMN Subscription(s) failed " "to delete."
