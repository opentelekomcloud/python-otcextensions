...
class ShowCheckpoint(command.ShowOne):
    ...
    columns = ("id", "status", "created_at", "vault_id", "vault_name", "backup_name")

    def get_parser(self, prog_name):
        parser = super(ShowCheckpoint, self).get_parser(prog_name)
        parser.add_argument(
            "checkpoint", metavar="<checkpoint>", help=_("ID of the CBR checkpoint.")
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.get_checkpoint(checkpoint=parsed_args.checkpoint)

        data = utils.get_dict_properties(_flatten_checkpoint(obj), self.columns)
...
class CreateCheckpoint(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--vault-id", metavar="<vault_id>", required=True, help=_("Vault id.")
        )
        parser.add_argument(
            "--auto-trigger",
            dest="auto_trigger",
        ...
        parser.add_argument(
            "--description", default="backup", help=_("Backup description.")
        )
        parser.add_argument(
            "--no-incremental",
            dest="no_incremental",
        ...
        parser.add_argument(
            "--backup-name", metavar="backup_name", help=_("Backup name.")
        )
        parser.add_argument(
            "--resources",
            action="append",
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.description:
            attrs["parameters"].update(description=parsed_args.description)
        ...
        if parsed_args.backup_name:
            attrs["parameters"].update(name=parsed_args.backup_name)
        if parsed_args.resources:
            attrs["parameters"].update(resources=parsed_args.resources)
        if parsed_args.resource_details:
            attrs["parameters"].update(
                resource_details=_normalize_resources(parsed_args.resource_details)
            )
        ...
        if obj.vault.skipped_resources:
            data, self.columns = _add_skipped_resources_to_obj(obj, data, self.columns)
