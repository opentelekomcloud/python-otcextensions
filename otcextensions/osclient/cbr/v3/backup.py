...
class ListBackups(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--dec", metavar="<dec>", type=bool, help=_("Dedicated cloud")
        )
        parser.add_argument(
            "--end-time", metavar="<end_time>", help=_("Time when the backup ends.")
        )
        parser.add_argument(
            "--image-type",
            metavar="<image_type>",
        ...
        parser.add_argument("--name", metavar="<name>", help=_("Backup name."))
        parser.add_argument(
            "--offset",
            type=int,
            metavar="<offset>",
        ...
        parser.add_argument(
            "--parent-id", metavar="<parent_id>", help=_("Parent backup ID.")
        )
        parser.add_argument(
            "--resource-az",
            metavar="<resource_az>",
        ...
        parser.add_argument(
            "--resource-id", metavar="<resource_id>", help=_("Resource ID.")
        )
        parser.add_argument(
            "--resource-name", metavar="<resource_name>", help=_("Resource name.")
        )
        parser.add_argument(
            "--resource-type",
        ...
        parser.add_argument("--vault-id", metavar="<vault_id>", help=_("Vault ID."))
        return parser
...
class ShowBackup(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "backup", metavar="<backup>", help=_("ID or name of the CBR backup.")
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.find_backup(name_or_id=parsed_args.backup, ignore_missing=False)

        data = utils.get_dict_properties(_flatten_backup(obj), self.columns)

        if obj.children:
            data, self.columns = _add_children_to_backup_obj(obj, data, self.columns)
...
class DeleteBackup(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        backup = client.find_backup(name_or_id=parsed_args.backup, ignore_missing=False)

        self.app.client_manager.cbr.delete_backup(
            backup=backup.id, ignore_missing=False
        )
...
class RestoreBackup(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument("backup", metavar="backup", help=_("Backup id."))
        parser.add_argument(
            "--mappings",
            metavar="<mappings>",
            action="append",
            help=_(
                'Restored mapping relationship  in "volume_id=volume_uuid'
        ...
        parser.add_argument(
            ...
            metavar="<server_id>",
            help=_(
                "ID of the target VM to be restored. This parameter is"
        ...
        parser.add_argument(
            ...
            metavar="<volume_id>",
            help=_(
                "ID of the target disk to be restored. This parameter is"
