...
class ListPolicies(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--vault-id", metavar="<vault_id>", help=_("The ID of the vault.")
        )
        parser.add_argument(
            "--operation-type",
...
class ShowPolicy(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "policy", metavar="<policy>", help=_("ID or name of the CBR policy.")
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.find_policy(policy_id=parsed_args.policy, ignore_missing=False)

        data = utils.get_dict_properties(_flatten_policy(obj), self.columns)

        if obj.associated_vaults:
            data, self.columns = _add_vaults_to_policy_obj(obj, data, self.columns)

        if obj.trigger.properties.pattern:
            data, self.columns = _add_scheduling_patterns(obj, data, self.columns)
...
class CreatePolicy(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument("name", metavar="<name>", help=_("Name of the CBR Policy."))
        parser.add_argument(
            "--disable",
            action="store_false",
    ...
    def take_action(self, parsed_args):
        ...
        attrs = {"operation_definition": {}, "trigger": {"properties": {"pattern": []}}}
        ...
        if parsed_args.day_backups:
            attrs["operation_definition"].update(day_backups=parsed_args.day_backups)
        if parsed_args.week_backups:
            attrs["operation_definition"].update(week_backups=parsed_args.week_backups)
        ...
        if parsed_args.year_backups:
            attrs["operation_definition"].update(year_backups=parsed_args.year_backups)
        if parsed_args.max_backups:
            attrs["operation_definition"].update(max_backups=parsed_args.max_backups)
        ...
        if (
            parsed_args.day_backups
            or parsed_args.week_backups
            or parsed_args.month_backups
            or parsed_args.year_backups
        ) and not parsed_args.timezone:
            msg = (
                "Parameter timezone must be provided if "
...
class UpdatePolicy(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--name", metavar="<name>", help=_("Name of the CBR Policy.")
        )
        parser.add_argument(
            "--disable",
            action="store_true",
        ...
        parser.add_argument(
            "--enable", action="store_true", help=_("Enables CBR Policy.")
        )
        parser.add_argument(
            "--pattern",
            metavar="<pattern>",
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.patterns:
            trigger = {"trigger": {"properties": {"pattern": parsed_args.patterns}}}
        ...
        client = self.app.client_manager.cbr
        policy = client.find_policy(policy_id=parsed_args.policy, ignore_missing=False)
...
class DeletePolicy(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        self.app.client_manager.cbr.delete_policy(
            policy=policy.id, ignore_missing=False
        )
