...
class ListVaults(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument("--id", metavar="<id>", help=_("Vault ID."))
        parser.add_argument("--name", metavar="<name>", help=_("Vault name"))
        parser.add_argument(
            "--cloud-type",
            metavar="<cloud_type>",
            help=_("Cloud type, which is public."),
        )
        parser.add_argument("--limit", type=int, metavar="<limit>", help=_("Limit."))
        parser.add_argument(
            "--object-type",
            metavar="<object_type>",
        ...
        parser.add_argument("--policy-id", metavar="<policy_id>", help=_("Policy ID."))
        parser.add_argument(
            "--protect-type",
            metavar="<protect_type>",
            help=_("Protection type, which is backup."),
        )
        parser.add_argument("--status", metavar="<status>", help=_("Status."))
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        def row_generator():
            for s in data:
                ...
                row_data = utils.get_dict_properties(_flatten_vault(s), row_columns)
                if s.resources:
...
class ShowVault(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "vault", metavar="<vault>", help=_("ID or name of the CBR vault.")
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.find_vault(name_or_id=parsed_args.vault, ignore_missing=False)

        data = utils.get_dict_properties(_flatten_vault(obj), self.columns)

        if obj.resources:
            data, self.columns = _add_resources_to_vault_obj(obj, data, self.columns)
        if obj.tags:
            data, self.columns = _add_tags_to_vault_obj(obj, data, self.columns)
...
class CreateVault(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument("name", metavar="<name>", help=_("Name of the CBR Vault."))
        parser.add_argument(
            "--backup-policy",
        ...
        parser.add_argument(
            ...
            help=_(
                "Backup specifications, which can be \n"
        ...
        parser.add_argument(
            "--console-url", metavar="<console_url>", help=_("Redirection URL.")
        )
        parser.add_argument(
            "--resource",
            ...
            optional_keys=["name"],
            help=_(
                'Associated resource in "id=resource_id,'
        ...
        parser.add_argument(
            ...
            action="append",
            help=_(
                "Tag to assign to the server in KEY=VALUE format. "
                "Repeat for multiple values."
            ),
        )
        return parser
...
class UpdateVault(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--name", metavar="<name>", help=_("Name of the CBR Vault.")
        )
        parser.add_argument(
            "--auto-bind",
            metavar="<auto_bind>",
        ...
        parser.add_argument(
            ...
            type=int,
            help=_(
                "Vault capacity threshold. If the vault capacity usage "
    ...
    def take_action(self, parsed_args):
        ...
        client = self.app.client_manager.cbr
        vault = client.find_vault(name_or_id=parsed_args.vault, ignore_missing=False)
...
class DeleteVault(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        self.app.client_manager.cbr.delete_vault(vault=vault.id, ignore_missing=False)
...
class AssociateVaultResource(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        if resources:
            obj = client.associate_resources(vault=vault.id, resources=resources)
...
class DissociateVaultResource(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            metavar="<resource>",
            action="append",
            help=_("Removed resource IDs." "Repeat for multiple values."),
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.resource:
            client.dissociate_resources(vault=vault.id, resources=parsed_args.resource)
...
class BindVaultPolicy(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "policy", metavar="<policy>", help=_("ID or name of the CBR Vault.")
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.policy:
            obj = client.bind_policy(vault=vault.id, policy=parsed_args.policy)
...
class UnbindVaultPolicy(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        client.unbind_policy(vault=vault.id, policy=parsed_args.policy)
