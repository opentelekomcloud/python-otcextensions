...
STATUS_CHOICES = ["PENDING_ACCEPTANCE", "REJECTED", "EXPIRED", "DELETED", "ACTIVE"]
...
def set_attributes_for_print(peerings):
    for peering in peerings:
        setattr(peering, "local_router_id", peering.local_vpc_info["vpc_id"])
        setattr(peering, "peer_router_id", peering.peer_vpc_info["vpc_id"])
        setattr(peering, "peer_project_id", peering.peer_vpc_info["tenant_id"])
        yield peering
...
def translate_response(func):
    def new(self, *args, **kwargs):
        ...
        setattr(response, "local_vpc_info", _update_vpc_info(response.local_vpc_info))
        setattr(response, "peer_vpc_info", _update_vpc_info(response.peer_vpc_info))
        columns = (
            "id",
            "name",
            "local_vpc_info",
...
class CreateVpcPeering(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            required=True,
            help=_(
                "Specifies information about the local router_id "
        ...
        parser.add_argument(
            ...
            required=True,
            help=_(
                "Specifies information about the peer router_id "
                "involved in a VPC peering connection."
            ),
        )
        parser.add_argument(
            ...
            help=_(
                "Specifies the ID of the project to which a "
    ...
    def take_action(self, parsed_args):
        ...
        attrs = {
            ...
            "request_vpc_info": {"vpc_id": parsed_args.local_router_id},
            "accept_vpc_info": {"vpc_id": parsed_args.peer_router_id},
        }
...
class UpdateVpcPeering(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--description",
            metavar="<description>",
            help=_("Specifies the new descrition of the VPC peering " "connection."),
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        for peering in parsed_args.peering:
            try:
                ...
                LOG.error(
                    _(
                        "Failed to delete VPC peering connection with "
        ...
        if result > 0:
            ...
            msg = _("%(result)s of %(total)s VPC peering(s) failed " "to delete.") % {
                "result": result,
                "total": total,
            }
