...
def _flatten_cluster_node(obj):
    ...
    data = {
        ...
        "volumes": (
            ";".join((x.disk_type + ":" + str(x.disk_size)) for x in obj.spec.volume)
            if obj.spec.volume
            else ""
        ),
        "conditions": (
...
class ShowCCEClusterNode(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.find_cluster_node(
            cluster=parsed_args.cluster, node=parsed_args.node
        )

        data = utils.get_dict_properties(_flatten_cluster_node(obj), self.columns)
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.cluster and parsed_args.node:
            ...
            client.delete_cluster_nodes(
                cluster=parsed_args.cluster, node_names=parsed_args.node
            )
...
class CreateCCEClusterNode(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "cluster", metavar="<cluster>", help=_("Name or ID of the cluster.")
        )
        parser.add_argument(
            "--flavor",
            metavar="<flavor>",
        ...
        parser.add_argument(
            "--label", metavar="<label>", help=_("The label attached to the node.")
        )
        parser.add_argument(
            "--volume",
            metavar="<volume>",
        ...
        parser.add_argument(
            ...
            choices=range(1, 15),
            default=1,
            help=_("Number of node instances to create " "(max 15 in the cluster)."),
        )
        return parser
