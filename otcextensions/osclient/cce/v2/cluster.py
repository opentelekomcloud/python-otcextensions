...
class ShowCCECluster(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument("cluster", metavar="<cluster>", help=_("ID of the cluster"))
        return parser
...
class DeleteCCECluster(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        attrs = {}
        for attr in ["cluster", "wait", "wait_timeout", "wait_interval"]:
        ...
        if parsed_args.cluster:
            self.app.client_manager.sdk_connection.delete_cce_cluster(**attrs)
...
class CreateCCECluster(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "router", metavar="<router>", help=_("ID or name the Neutron Router.")
        )
        parser.add_argument(
            "network", metavar="<network>", help=_("ID or name of the Neutron network.")
        )
        parser.add_argument(
            "--description",
            metavar="<description>",
        ...
        parser.add_argument(
            "--multi-az", action="store_true", help=_("Support for multi-AZ cluster")
        )
        parser.add_argument(
            "--version", metavar="<version>", help=_("Cluster version.")
        )
        parser.add_argument(
            "--flavor", metavar="<flavor>", required=True, help=_("Cluster flavor.")
        )
        parser.add_argument(
            "--type",
    ...
    def take_action(self, parsed_args):
        ...
        # initialize sdk_connection with cce methods
        _ = self.app.client_manager.cce
        obj = self.app.client_manager.sdk_connection.create_cce_cluster(**attrs)
