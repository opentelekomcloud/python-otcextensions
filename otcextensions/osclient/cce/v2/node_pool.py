...
#   under the License.
#
"""CCE Node Pool CLI implementation"""
...
class ShowCCENodePool(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "nodepool", metavar="<nodepool>", help=_("ID of the CCE node pool.")
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.find_node_pool(cluster=cluster.id, node_pool=parsed_args.nodepool)

        data = utils.get_dict_properties(_flatten_node_pool(obj), self.columns)
...
class CreateCCENodePool(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "name", metavar="<name>", help=_("Name of the CCE Node Pool.")
        )
        parser.add_argument(
            "--autoscaling-enabled",
        ...
        parser.add_argument(
            ...
            metavar="<ecs_group>",
            help=_(
                "ECS group id of the ECS group to which nodes belong " "after creation."
            ),
        )
        parser.add_argument(
            "--flavor",
        ...
        parser.add_argument(
            ...
            type=int,
            help=_(
                "If auto-scaling is enabled, the value describes the "
        ...
        parser.add_argument(
            ...
            help=_("Mandatory if custom image is used on a bare " "metall node."),
        )
        parser.add_argument(
            "--node-pool-type",
        ...
        parser.add_argument(
            ...
            help=_(
                "Script required before installation. " "Input must be Base64 encoded."
            ),
        )
        parser.add_argument(
            "--postinstall-script",
            ...
            help=_(
                "Script required after installation. " "Input must be Base64 encoded."
            ),
        )
        parser.add_argument(
            "--priority",
        ...
        parser.add_argument(
            ...
            required=True,
            help=_(
                "ID or name of the network to which the cluster node " "will belong to."
            ),
        )
        parser.add_argument(
            "--root-volume-size",
    ...
    def take_action(self, parsed_args):
        ...
        obj = self.app.client_manager.sdk_connection.create_cce_node_pool(**attrs)
...
class DeleteCCENodePool(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            nargs="+",
            help=_(
                "Name or ID of CCE Node pool. Repeat names with spaces "
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.cluster and parsed_args.nodepool:
            ...
            for pool in parsed_args.nodepool:
                obj = client.find_node_pool(cluster=cluster.id, node_pool=pool)
                client.delete_node_pool(
                    cluster=cluster.id, node_pool=obj.id, ignore_missing=False
                )
