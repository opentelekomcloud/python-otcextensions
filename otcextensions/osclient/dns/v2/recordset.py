...
class ListRS(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            metavar="<zone>",
            help=_(
                "UUID or name of the zone. Recordsets of all zones "
                "will be returned if not given."
            ),
        )
        parser.add_argument("--zone-type", help=_("DNS Zone type (private/public)"))
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.zone:
            ...
            zone = client.find_zone(parsed_args.zone, ignore_missing=False, **attrs)
...
class ShowRS(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "zone", metavar="<zone>", help=_("UUID or name of the zone.")
        )
        parser.add_argument("--zone-type", help=_("DNS Zone type (private/public)"))

        parser.add_argument(
            "recordset", metavar="<rs>", help=_("UUID or name of the recordset.")
        )
    ...
    def take_action(self, parsed_args):
        ...
        zone = client.find_zone(
            parsed_args.zone, ignore_missing=False, zone_type=parsed_args.zone_type
        )

        obj = client.find_recordset(zone=zone, name_or_id=parsed_args.recordset)
    ...
    def take_action(self, parsed_args):
        if parsed_args.zone:
            ...
            for rs in parsed_args.recordset:
                rs = client.find_recordset(
                    zone=zone, name_or_id=rs, ignore_missing=False
                )
                client.delete_recordset(recordset=rs, zone=zone, ignore_missing=False)
...
class CreateRS(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument("--zone-type", help=_("DNS Zone type (private/public)"))
        parser.add_argument(
            "--name", metavar="<name>", required=True, help=_("DNS Name for the zone.")
        )
        parser.add_argument(
            "--description",
            metavar="<description>",
        ...
        parser.add_argument(
            ...
            action="append",
            help=_(
                "Record set value, which varies depending on the record "
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.create_recordset(zone=zone, **attrs)
...
class SetRS(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        recordset = client.find_recordset(zone=zone, name_or_id=parsed_args.recordset)
