...
class ListPool(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            help=_(
                "Load balancer pool algorithm to query"
                "one of [`LEAST_CONNECTIONS`, `ROUND_ROBIN`, `SOURCE_IP`]"
            ),
        )
        parser.add_argument(
            "--name", metavar="<name>", help=_("Load balancer pool name to query")
        )
        parser.add_argument(
            "--protocol",
            ...
            help=_(
                "Load balancer pool protocol to query"
...
class ShowPool(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "pool", metavar="<pool>", help=_("Load balancer pool id to show.")
        )
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.find_pool(
            name_or_id=parsed_args.pool, ignore_missing=False, **args
        )
...
class CreatePool(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument("--name", metavar="<name>", help=_("Set pool name."))
        parser.add_argument(
            "--description", metavar="<description>", help=_("Set pool description.")
        )
        parser.add_argument(
            "--protocol",
            metavar="{" + ",".join(PROTOCOL_VALUES) + "}",
        ...
        group.add_argument(
            ...
            metavar="<listener>",
            help=_(
                "The ID of the listener for the pool. "
        ...
        group.add_argument(
            ...
            help=_(
                "The ID of the loadbalancer for the pool. "
                "Either listener_id or loadbalancer_id must be specified."
            ),
        )
        parser.add_argument(
            ...
            help=_(
                "A JSON object specifying the session persistence "
        ...
        admin_group.add_argument(
            "--disable", action="store_true", default=None, help=_("Disable pool.")
        )
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.session_persistence is not None:
            args["session_persistence"] = json.loads(parsed_args.session_persistence)
...
class SetPool(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "pool", metavar="<pool>", help=_("The ID of the pool to update.")
        )
        parser.add_argument(
            "--name", metavar="<name>", help=_("Human-readable name of the resource.")
        )
        parser.add_argument(
            "--description",
            metavar="<description>",
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.update_pool(pool=parsed_args.pool, **args)
...
class DeletePool(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "pool", metavar="<pool>", nargs="+", help=_("The ID of the pool to delete.")
        )
