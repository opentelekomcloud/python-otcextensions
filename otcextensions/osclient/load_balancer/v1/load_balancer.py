...
OPERATING_STATUS = ["ONLINE", "DRAINING", "OFFLINE", "DEGRADED", "ERROR", "NO_MONITOR"]
...
class ShowLoadBalancer(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.find_load_balancer(
            name_or_id=parsed_args.load_balancer, ignore_missing=False
        )
...
class CreateLoadBalancer(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--name", metavar="<name>", help=_("New load balancer name.")
        )
        parser.add_argument(
            "--description",
            metavar="<description>",
        ...
        parser.add_argument(
            "--vip_address", metavar="<vip_address>", help=_("Set the VIP IP Address.")
        )
...
class SetLoadBalancer(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        admin_group = parser.add_mutually_exclusive_group()
        admin_group.add_argument(
            "--enable", action="store_true", default=None, help="Enable load balancer."
        )
        admin_group.add_argument(
            "--disable",
            action="store_true",
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.update_load_balancer(
            load_balancer=parsed_args.load_balancer, **args
        )
...
class DeleteLoadBalancer(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        for lb in parsed_args.load_balancer:
            ...
            client.delete_load_balancer(load_balancer=obj.id, ignore_missing=False)
