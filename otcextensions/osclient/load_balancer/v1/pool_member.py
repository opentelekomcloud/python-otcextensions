...
class ListPoolMember(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "pool", metavar="<pool>", help=_("Pool name or ID to list the members of.")
        )
        parser.add_argument(
            "--name",
            metavar="<name>",
...
class ShowPoolMember(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "pool", metavar="<pool>", help=_("Pool name or ID to show the members of.")
        )
        parser.add_argument(
            "member", metavar="<member>", help=_("Name or ID of the member to show.")
        )
...
class CreatePoolMember(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument("--name", metavar="<name>", help=_("Name of the member."))
        parser.add_argument(
            "--weight",
            metavar="<weight>",
            type=int,
        ...
        parser.add_argument(
            ...
            metavar="<ip_address>",
            help=_(
                "The IP address of the backend member to receive traffic "
        ...
        admin_group.add_argument(
            "--disable", action="store_true", default=None, help=_("Disable member.")
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.create_pool_member(pool=parsed_args.pool, **args)
...
class SetPoolMember(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--name", metavar="<name>", help=_("Name or ID of the member to update.")
        )
        parser.add_argument(
            "--weight",
            metavar="<weight>",
            ...
            choices=range(0, 256),
            help=_(
                "The weight of a member determines the portion of requests "
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.update_pool_member(
            pool_member=parsed_args.member, pool=parsed_args.pool, **args
        )
...
class DeletePoolMember(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        for member in parsed_args.member:
            obj = client.find_pool_member(
                name_or_id=member, pool=parsed_args.pool, ignore_missing=False
            )
            client.delete_pool_member(
                pool_member=obj.id, pool=parsed_args.pool, ignore_missing=False
            )
