...
class AssumeRoleMethod(base.AuthMethod):
    ...
    def get_cache_id_elements(self):
        return dict(
            ("assume_role_%s" % p, getattr(self, p)) for p in self._method_parameters
        )
...
class Agency(base.AuthConstructor):
    ...
    def __init__(self, auth_url, *args, **kwargs):
        super(Agency, self).__init__(auth_url=auth_url, *args, **kwargs)
    ...
    def get_auth_ref(self, session, **kwargs):
        ...
        # If project is not set - get a domain scope
            ...
            body["auth"]["scope"] = {"domain": {"id": self.target_domain_id}}
        elif self.target_domain_name:
            body["auth"]["scope"] = {"domain": {"name": self.target_domain_name}}
    ...
    def get_options(self, **kwargs):
        ...
        options.extend(
            [
                ...
                loading.Opt(
                    "username", help="Username", deprecated=[loading.Opt("user-name")]
                ),
                loading.Opt("user-domain-id", help="User's domain id"),
                loading.Opt("user-domain-name", help="User's domain name"),
                loading.Opt(
                    "password", secret=True, prompt="Password: ", help="User's password"
                ),
                loading.Opt("target-agency-name", help="Agency name"),
                loading.Opt(
                    "target-project-id", help="Project id available through agency"
                ),
                loading.Opt(
                    "target-project-name", help="Project name available through agency"
                ),
                loading.Opt(
                    "target-domain-id", help="Domain id available through agency"
                ),
                loading.Opt(
                    "target-domain-name", help="Domain name available through agency"
                ),
                loading.Opt("roles", help="List of the roles to request from agency"),
            ]
        )
        return options

    def load_from_options(self, **kwargs):
        if kwargs.get("username") and not (
            kwargs.get("user_domain_name") or kwargs.get("user_domain_id")
        ):
            m = (
        ...
        if not kwargs.get("target_agency_name") or not (
            kwargs.get("target_domain_id") or kwargs.get("target_domain_name")
        ):
            m = (
                "Using agency based authorization requires "
