...
def _get_file_segments(endpoint, filename, file_size, segment_size):
    ...
    for index, offset in enumerate(range(0, file_size, segment_size)):
        ...
        segment = FileSegment(
            filename, offset, segment_size if segment_size < remaining else remaining
        )
        name = "{endpoint}/{index}".format(endpoint=endpoint, index=index + 1)
...
def extract_url_parts(url: str, project_id: str) -> list:
    ...
    url_parts = list(
        filter(
            lambda x: len(x) > 0
            and not (x.lower().startswith("v") and x[1:].isdigit()),
            url_parts,
        )
    )
