...
class Cluster(resource.Resource):
    ...
    def update_flavor(self, session, new_flavor, node_type=None, check_replica=True):
        """Modify cluster specifications."""
        body = {"newFlavorId": new_flavor}
    ...
    def scale_in(self, session, nodes):
        ...
        if not isinstance(nodes, list):
            raise exceptions.SDKException("nodes parameter should be list type")
    ...
    def replace_node(self, session, node_id):
        ...
        uri = utils.urljoin("clusters", self.id, "instance", node_id, "replace")
        ...
        if has_body:
            try:
                ...
                if "status" in body.keys():
                    ...
                    body["status"] = STATUS_MAP.get(str(body["status"]), "ERROR")
