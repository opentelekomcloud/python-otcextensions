...
class Stream(resource.Resource):
    ...
    auto_scale_min_partition_count = resource.Body(
        "auto_scale_min_partition_count", type=int
    )
    #: Maximum number of partitions for automatic scale-up when
    #:  auto scaling is enabled.
    auto_scale_max_partition_count = resource.Body(
        "auto_scale_max_partition_count", type=int
    )
    ...
    current_partition_count = resource.Body("current_partition_count", type=int)
    ...
    readable_partition_count = resource.Body("readable_partition_count", type=int)
    ...
    writable_partition_count = resource.Body("writable_partition_count", type=int)
        ...
        **params
        ...
        while uri:
            ...
            if resources and paginated:
                uri, next_params = cls._get_next_link(uri, response, data, limit)
