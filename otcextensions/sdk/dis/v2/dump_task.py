...
class ObsDestinationSpec(resource.Resource):
    ...
    processing_schema = resource.Body("processing_schema", type=ProcessingSchemaSpec)
...
class DumpTask(resource.Resource):
    ...
    obs_destination_descriptor = resource.Body(
        "obs_destination_descriptor", type=ObsDestinationSpec
    )
    #: Parameter list of OBS to which data in the DIS stream will be dumped.
    obs_destination_description = resource.Body(
        "obs_destination_description", type=ObsDestinationSpec
    )
    ...
    def _action(self, session, stream_name, action, *task_id):
        ...
        body = {"action": action, "tasks": tasks}
