...
class CceMixin:
    ...
    def create_cce_cluster(
        self, wait=True, wait_timeout=1800, wait_interval=5, **kwargs
    ):
        """Create CCE cluster
        ...
        if cluster_type.lower() not in ("virtualmachine"):
            raise ValueError("CCE Cluster type %s is not supported" % cluster_type)
        ...
        if authentication_mode or authentication_proxy_ca:
            auth = {}
            if authentication_mode and authentication_mode in (
                "x509",
                "rbac",
                ...
                raise ValueError(
                    "Authentication mode %s is not supported" % authentication_mode
                )
        ...
        if container_network_mode not in (
            "overlay_l2",
            "underlay_ipvlan",
            "vpc-router",
        ):
            raise ValueError(
                "Container network mode %s is not supported" % container_network_mode
            )
        ...
        if cpu_manager_policy:
            extended_params["kubernetes.io/cpuManagerPolicy"] = cpu_manager_policy
        if upgrade_from:
        ...
        host_network = {"vpc": router_obj.id, "subnet": network_obj.id}
        spec["hostNetwork"] = host_network

        obj = self.cce.create_cluster(metadata=metadata, spec=spec)
    ...
    def create_cce_cluster_node(
        ...
        data_volumes=[
            {"volumetype": "SATA", "size": 100, "encrypted": False, "cmk_id": ""}
        ],
        wait=True,
        wait_timeout=300,
        wait_interval=5,
        **kwargs
        ...
        if root_volume_size:
            ...
            if root_volume_size < 40:
                raise ValueError(
                    "Root volume size %s is lower than 40 GB." % root_volume_size
                )
            spec["rootVolume"]["size"] = root_volume_size
        if root_volume_type.upper() not in volume_types:
            raise ValueError(
                "Root volume type %s is not supported, use: %s" % root_volume_type,
                volume_types,
            )
        ...
        for item in data_volumes:
            if not (item["volumetype"] and item["size"]):
                raise ValueError(
                    "One or both data volume keys: volumetype " "or size are missing."
                )
            if item["volumetype"] not in volume_types:
                raise ValueError(
                    "The data volumes volumetype %s must be "
                    "one of the following choices: %s" % item["volumetype"],
                    volume_types,
                )
            if not isinstance(item["size"], int):
                try:
                    ...
                    print(
                        "data_volume size %s cannot be converted into "
                        "integer value." % item["size"]
                    )
            if not (100 <= item["size"] <= 32768):
                raise ValueError(
                    "The data volume size must be specified "
                    "between 100 and 32768 GB."
                )
            if ("encrypted" in item) and item["encrypted"]:
                if not item["cmk_id"]:
                    raise ValueError(
                        "Parameter cmk_id is missing to use " "data volume encryption."
                    )
                volumes.append(
                    {
                ...
                volumes.append(
                    {"volumetype": item["volumetype"].upper(), "size": item["size"]}
                )
        if volumes:
        ...
        if dedicated_host:
            deh = self.deh.find_host(name_or_id=dedicated_host, ignore_missing=False)
        ...
        if floating_ips and isinstance(floating_ips, list):
            ...
            if not (count == len(floating_ips)):
                raise ValueError(
                    "The length of the list floating_ip_ids is"
        ...
        if fip_count and bandwidth:
            ...
            if floating_ips:
                raise ValueError(
                    "floating_ips is used together with"
        ...
        nw = self.network.find_network(network, ignore_missing=False)
        spec["nodeNicSpec"]["primaryNic"]["subnetId"] = nw.id
        if tags:
            spec["userTags"] = tags

        cluster = self.cce.find_cluster(name_or_id=cce_cluster, ignore_missing=True)
        if not cluster:
            raise ReferenceError("Cluster %s not found." % cce_cluster)
        obj = self.cce.create_cluster_node(
            cluster=cluster.id, metadata=metadata, spec=spec
        )
        ...
        if obj.job_id and wait:
            ...
            self.cce.wait_for_job(obj.job_id, **wait_args)
            obj = self.cce.get_cluster_node(cluster=cluster.id, node_id=obj.id)

        return obj

    def delete_cce_cluster_node(
        self, wait=True, wait_timeout=180, wait_interval=5, **kwargs
    ):
        ...
        cluster = self.cce.find_cluster(name_or_id=cluster, ignore_missing=False)
        node = self.cce.find_cluster_node(cluster=cluster, node=node)

        obj = self.cce.delete_cluster_node(cluster=cluster, node=node)
    ...
    def create_cce_node_pool(
        ...
        **kwargs
        ...
        if not (cce_cluster and flavor and os and name and network and ssh_key):
            raise ValueError(
        ...
        if tags:
            ...
            for item in tags:
                ...
                if not isinstance(item, dict):
                    raise ValueError(
                        "One or more tags items %s are not "
                        "formatted as dicts." % item
                    )
                if not (set(["key", "value"]) <= item.keys()):
                    raise ValueError(
                        "The current tags list item %s has "
                        "wrong format. Each dict item has to "
                        "provide the keys: key and value." % item
                    )
        ...
        if taints:
            ...
            for item in taints:
                ...
                if not isinstance(item, dict):
                    raise ValueError(
                        "Each list item of taints parameter has " "to be a dict."
                    )
                if not (set(["key", "value", "effect"]) <= item.keys()):
                    raise ValueError(
                        "Each taints list item must provide "
                        "the following keys: key, value, "
                        "effect."
                    )
                if not (1 <= len(item["key"]) <= 63):
                    raise ValueError(
                        "taints key %s exceeds character " "range." % item["key"]
                    )
                if not (1 <= len(item["value"]) <= 63):
                    raise ValueError(
                        "taints value %s exceeds character " "range." % item["value"]
                    )
                if item["effect"] not in effects:
                    raise ValueError(
                        "taints effect %s is different from the "
                        "possible options: NoSchedule, "
                        "PrefereNoSchedule, NoExecute." % item["effect"]
                    )
        ...
        # Node pool specs
        spec = {"autoscaling": {}, "nodeManagement": {}, "nodeTemplate": node_template}
        ...
        if autoscaling_enabled:
            ...
            if min_node_count:
                if not isinstance(min_node_count, int):
                    raise ValueError("min_node_count is not an integer " "value.")
                spec["autoscaling"]["minNodeCount"] = min_node_count
            if max_node_count:
                if not isinstance(max_node_count, int):
                    raise ValueError("max_node_count is not an integer " "value.")
                spec["autoscaling"]["maxNodeCount"] = max_node_count
            if priority:
                if not isinstance(priority, int):
                    raise ValueError("priority is not an integer " "value.")
                if priority > 1:
                ...
                if scale_down_cooldown_time:
                    if not isinstance(scale_down_cooldown_time, int):
                        raise ValueError(
                            "scale_down_cooldown_time is not an " "integer value."
                        )
        ...
        if initial_node_count:
            if not isinstance(initial_node_count, int):
                raise ValueError("initial_node_count is not an integer " "value.")
    ...
    def delete_cce_node_pool(self, **kwargs):
        ...
        cluster = self.cce.find_cluster(name_or_id=cluster, ignore_missing=False)
        node_pool = self.cce.find_node_pool(cluster=cluster, node_pool=node_pool)

        self.cce.delete_node_pool(cluster=cluster, node_pool=node_pool)
