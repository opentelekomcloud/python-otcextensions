...
class MemberGroupCreateSpec(resource.Resource):
    ...
    microservice_labels = resource.Body(
        "microservice_labels", type=list, list_type=MicroserviceLabelSpec
    )
...
class MicroServiceCreateSpec(resource.Resource):
    ...
    cse_info = resource.Body("cse_info", type=MicroServiceInfoCSEBaseSpec)
    cce_info = resource.Body("cce_info", type=MicroServiceInfoCCEBaseSpec)
    id = resource.Body("id")
...
class VpcChannel(resource.Resource):
    base_path = "apigw/instances/%(gateway_id)s/vpc-channels"
    ...
    dict_code = resource.Body("dict_code")
    member_groups = resource.Body(
        "member_groups", type=list, list_type=MemberGroupCreateSpec
    )
    members = resource.Body("members", type=list, list_type=MemberInfoSpec)
    vpc_health_config = resource.Body("vpc_health_config", type=VpcHealthConfigSpec)
    microservice_info = resource.Body("microservice_info", type=MicroServiceCreateSpec)
    create_time = resource.Body("create_time")
    id = resource.Body("id")
    status = resource.Body("status", type=int)
