...
class AclApiBinding(resource.Resource):
    base_path = "apigw/instances/%(gateway_id)s/acl-bindings"
    ...
    def _bind_to_api(self, session, gateway_id, **attrs):
        ...
        for raw_resource in resources:
            value = self.existing(connection=session._get_connection(), **raw_resource)
            yield value
...
class ApiForAcl(resource.Resource):
    base_path = "apigw/instances/%(gateway_id)s/acl-bindings/binded-apis"
    gateway_id = resource.URI("gateway_id", type=str)
    _query_mapping = resource.QueryParameters(
        "limit", "offset", "acl_id", "api_id", "api_name", "env_id", "group_id"
    )
    allow_list = True
    resources_key = "apis"
    api_id = resource.Body("api_id")
...
class UnbindApiForAcl(resource.Resource):
    base_path = "apigw/instances/%(gateway_id)s/acl-bindings/unbinded-apis"
    gateway_id = resource.URI("gateway_id", type=str)
    _query_mapping = resource.QueryParameters(
        "limit", "offset", "acl_id", "api_id", "api_name", "env_id", "group_id"
    )
...
class AclForApi(resource.Resource):
    base_path = "apigw/instances/%(gateway_id)s/acl-bindings/binded-acls"
    gateway_id = resource.URI("gateway_id", type=str)
    _query_mapping = resource.QueryParameters(
        "limit", "offset", "acl_id", "api_id", "env_name", "env_id", "group_id"
    )
    allow_list = True
