...
class BackendServer(resource.Resource):
    base_path = "apigw/instances/%(gateway_id)s/vpc-channels/" "%(vpc_chan_id)s/members"
    ...
    def create_members(self, session, gateway_id, vpc_channel_id, **attrs):
        return self.send_request(session, gateway_id, vpc_channel_id, "POST", **attrs)

    def update_members(self, session, gateway_id, vpc_channel_id, **attrs):
        return self.send_request(session, gateway_id, vpc_channel_id, "PUT", **attrs)

    @staticmethod
    def send_request(session, gateway_id, vpc_channel_id, method, **attrs):
        uri = f"/apigw/instances/{gateway_id}/vpc-channels/" f"{vpc_channel_id}/members"
