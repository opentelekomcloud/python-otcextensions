...
class Notification(resource.Resource):
    ...
    max_async_retry_attempts = resource.Body("max_async_retry_attempts", type=int)
    ...
    enable_async_status_log = resource.Body("enable_async_status_log", type=str)
...
class Requests(resource.Resource):
    ...
    _query_mapping = resource.QueryParameters(
        "request_id", "marker", "limit", "status", "query_begin_time", "query_end_time"
    )
    ...
    def _stop(self, session, function_urn, **attrs):
        ...
        url = "fgs/functions/%(function_urn)s/cancel" % {"function_urn": function_urn}
