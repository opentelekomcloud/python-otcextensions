...
_DOC_TEMPLATE = ":class:`{class_name}` for {service_type} aka project"
...
OTC_SERVICES = {
    ...
    "aomv1": {"service_type": "aomv1", "endpoint_service_type": "aomv1"},
    "aomv2": {"service_type": "aomv2", "endpoint_service_type": "aomv2"},
    "auto_scaling": {
        "service_type": "auto_scaling",
        "endpoint_service_type": "as",
        "append_project_id": True,
    },
    "bms": {"service_type": "bms"},
    "cbr": {
        "service_type": "cbr",
        "append_project_id": True,
    },
    ...
    "cts": {"service_type": "cts", "endpoint_service_type": "cts"},
    "ctsv2": {"service_type": "ctsv2", "endpoint_service_type": "ctsv2"},
    "ctsv3": {"service_type": "ctsv3", "endpoint_service_type": "ctsv3"},
    "css": {
        "service_type": "css",
    },
    "csbs": {"service_type": "csbs", "endpoint_service_type": "data-protect"},
    "dataarts": {
    ...
    "dis": {"service_type": "dis", "endpoint_service_type": "disv2"},
    "dms": {
        "service_type": "dms",
    ...
    "dws": {"service_type": "dws", "endpoint_service_type": "dwsv1"},
    "ecs": {
        "service_type": "ecs",
    },
    "elb": {
    ...
    "identity": {"service_type": "identity", "replace_system": True},
    "imsv1": {
        "service_type": "imsv1",
    },
    "imsv2": {
    ...
    "rts": {"service_type": "rts", "endpoint_service_type": "orchestration"},
    "rms": {"service_type": "rms"},
    "nat": {
        "service_type": "nat",
    },
    "natv3": {
    ...
    "smn": {"service_type": "smn", "append_project_id": True},
    "swr": {"service_type": "swr", "endpoint_service_type": "swrv2"},
    ...
    "wafd": {"service_type": "wafd", "endpoint_service_type": "premium-waf"},
}
    ...
    if service:
        ...
        if not desc_class.supported_versions:
            doc = _DOC_TEMPLATE.format(
                class_name="{service_type} Proxy".format(service_type=service_type),
                **service
            )
        elif len(desc_class.supported_versions) == 1:
            supported_version = list(desc_class.supported_versions.keys())[0]
            doc = _DOC_TEMPLATE.format(
            ...
            class_doc_strings = "\n".join(
                [
                    ":class:`{class_name}`".format(class_name=proxy_class.__name__)
                    for proxy_class in desc_class.supported_versions.values()
                ]
            )
            doc = _PROXY_TEMPLATE.format(class_doc_strings=class_doc_strings, **service)
...
def get_ak_sk(conn):
    ...
    if not (ak and sk):
        _logger.error(
            "AK/SK pair is not configured in the connection, "
...
def patch_openstack_resources():
    openstack.proxy.Proxy._report_stats_statsd = proxy.Proxy._report_stats_statsd
    openstack.proxy.Proxy._report_stats_influxdb = proxy.Proxy._report_stats_influxdb
    openstack.compute.v2.server.Server._get_tag_struct = server.Server._get_tag_struct
    ...
    openstack.exceptions.raise_from_response = exc.raise_from_response
...
def register_single_service(conn, service_name, project_id=None, service=None):
    ...
    if service.get("append_project_id", False):
        ...
        if (
            ep
            and not ep.rstrip("/").endswith("\\%(project_id)s")
            and not ep.rstrip("/").endswith("$(tenant_id)s")
            and not ep.rstrip("/").endswith(project_id)
        ):
            key = "_".join(
                [sd.service_type.lower().replace("-", "_"), "endpoint_override"]
            )
            if key not in conn.config.config:
                conn.config.config[key] = utils.urljoin(ep, "%(project_id)s")
        ...
        ep = conn.endpoint_for(sd.service_type)
        conn.config.config[
            "_".join([sd.service_type.lower().replace("-", "_"), "endpoint_override"])
        ] = utils.urljoin(ep)
