...
class Resource(resource.Resource):
    ...
    def create(self, session, prepend_key=False, uri=None, requires_id=True, **params):
        ...
        response = session.post(
            request.url, json=request.body, headers=request.headers, params=params
        )
        ...
        if response.status_code == 400:
            ...
            if error:
                if error["error_code"] == "KMS.1104":
                    msg = _("Key with given alias %s already exist") % self.key_alias
                    raise exceptions.DuplicateResource(msg)
                else:
                    msg = (
                        _("Service returned an error with code=%s")
                        % error["error_code"]
                    )
            ...
            if has_body:
                ...
                if error:
                    ...
                    if error_code == "KMS.0205":
                        ...
                        msg = _("Service returned an error with code = %s") % error_code
                        raise exceptions.SDKException(msg)
