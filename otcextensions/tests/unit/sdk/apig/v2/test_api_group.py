...
class TestApiGroup(base.TestCase):
    ...
    def test_basic(self):
        ...
        self.assertEqual("/apigw/instances/%(gateway_id)s/api-groups", sot.base_path)
    ...
    def test_make_it(self):
        ...
        self.assertEqual(EXAMPLE_API_GROUP["on_sell_status"], sot.on_sell_status)
        self.assertEqual(
            EXAMPLE_API_GROUP["sl_domain_access_enabled"], sot.sl_domain_access_enabled
        )
        ...
        self.assertEqual(len(EXAMPLE_API_GROUP["url_domains"]), len(sot.url_domains))
        self.assertEqual(
            EXAMPLE_API_GROUP["url_domains"][0]["id"], sot.url_domains[0].id
        )
        self.assertEqual(
            EXAMPLE_API_GROUP["url_domains"][0]["domain"], sot.url_domains[0].domain
        )
        self.assertEqual(
        ...
        self.assertEqual(
            EXAMPLE_API_GROUP["url_domains"][0]["ssl_id"], sot.url_domains[0].ssl_id
        )
        self.assertEqual(
            EXAMPLE_API_GROUP["url_domains"][0]["ssl_name"], sot.url_domains[0].ssl_name
        )
        self.assertEqual(
