...
EXAMPLE_CUSTOM_AUTHORIZER = {
    ...
    "identities": [{"name": "Authorization", "location": "HEADER", "validation": ".*"}],
    "ttl": 300,
...
class TestCustomAuthorizer(base.TestCase):
    ...
    def test_basic(self):
        ...
        self.assertEqual("/apigw/instances/%(gateway_id)s/authorizers", sot.base_path)
    ...
    def test_make_it(self):
        ...
        self.assertEqual(EXAMPLE_CUSTOM_AUTHORIZER["gateway_id"], sot.gateway_id)
        ...
        self.assertEqual(
            EXAMPLE_CUSTOM_AUTHORIZER["authorizer_type"], sot.authorizer_type
        )
        self.assertEqual(
            EXAMPLE_CUSTOM_AUTHORIZER["authorizer_uri"], sot.authorizer_uri
        )
        self.assertEqual(EXAMPLE_CUSTOM_AUTHORIZER["network_type"], sot.network_type)
        self.assertEqual(
            EXAMPLE_CUSTOM_AUTHORIZER["authorizer_version"], sot.authorizer_version
        )
        self.assertEqual(
            EXAMPLE_CUSTOM_AUTHORIZER["authorizer_alias_uri"], sot.authorizer_alias_uri
        )
        ...
        self.assertEqual(EXAMPLE_CUSTOM_AUTHORIZER["create_time"], sot.create_time)
        self.assertEqual(EXAMPLE_CUSTOM_AUTHORIZER["roma_app_id"], sot.roma_app_id)
        self.assertEqual(EXAMPLE_CUSTOM_AUTHORIZER["roma_app_name"], sot.roma_app_name)
        self.assertEqual(
            len(EXAMPLE_CUSTOM_AUTHORIZER["identities"]), len(sot.identities)
        )
        self.assertEqual(
            EXAMPLE_CUSTOM_AUTHORIZER["identities"][0]["name"], sot.identities[0].name
        )
        self.assertEqual(
