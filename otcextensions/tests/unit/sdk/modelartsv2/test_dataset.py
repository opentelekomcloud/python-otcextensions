...
EXAMPLE_CREATE = {
    ...
    "dataset_type": 0,
    "data_sources": [
        {"data_type": 0, "data_path": "/test-obs/classify/input/cat-rabbit/"}
    ],
    "description": "",
...
class TestDataset(base.TestCase):
    ...
    def test_make_it(self):
        ...
        for data_source in EXAMPLE["data_sources"]:
            sot_data_source = dataset.DataSourceSpec(**data_source)
            for key, value in data_source.items():
                self.assertEqual(getattr(sot_data_source, key), value)

        for label in EXAMPLE["labels"]:
            sot_label = dataset.LabelSpec(**label)
            for key, value in label.items():
                if key == "property":
                    sot_property = dataset.LabelPropertySpec(**value)
                    self.assertEqual(
                        sot_property.color, value["@modelarts:color"]
                    )
                else:
                    self.assertEqual(getattr(sot_label, key), value)

    def test_make_it_create(self):
        updated_sot_attrs = [
            "data_sources",
            "labels",
        ]
        ...
        for data_source in EXAMPLE_CREATE["data_sources"]:
            sot_data_source = dataset.DataSourceSpec(**data_source)
            for key, value in data_source.items():
                self.assertEqual(getattr(sot_data_source, key), value)

        for label in EXAMPLE_CREATE["labels"]:
            sot_label = dataset.LabelSpec(**label)
            for key, value in label.items():
                if key == "property":
                    sot_property = dataset.LabelPropertySpec(**value)
