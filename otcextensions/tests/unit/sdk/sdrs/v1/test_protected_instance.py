...
EXAMPLE = {
    ...
    "attachment": [{"replication": uuid.uuid4(), "device": uuid.uuid4()}],
    "tags": [{"key": uuid.uuid4(), "value": uuid.uuid4()}],
    "metadata": {"__system__frozen": False},
}
...
class TestProtectedInstance(base.TestCase):
    ...
    def test_make_it(self):
        ...
        self.assertEqual(EXAMPLE["description"], test_protected_instance.description)
        self.assertEqual(
            EXAMPLE["server_group_id"], test_protected_instance.server_group_id
        )
        ...
        self.assertEqual(
            EXAMPLE["source_server"], test_protected_instance.source_server
        )
        self.assertEqual(
            EXAMPLE["target_server"], test_protected_instance.target_server
        )
        self.assertEqual(EXAMPLE["created_at"], test_protected_instance.created_at)
        self.assertEqual(EXAMPLE["updated_at"], test_protected_instance.updated_at)
        self.assertEqual(
            EXAMPLE["priority_station"], test_protected_instance.priority_station
        )
        self.assertEqual(
        ...
        self.assertEqual(EXAMPLE["tags"][0]["key"], test_protected_instance.tags[0].key)
        self.assertEqual(
            EXAMPLE["tags"][0]["value"], test_protected_instance.tags[0].value
        )
        self.assertEqual(
