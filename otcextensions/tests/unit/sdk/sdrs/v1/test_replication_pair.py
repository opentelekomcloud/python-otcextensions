...
EXAMPLE = {
    ...
    "attachment": [{"protected_instance": uuid.uuid4(), "device": "/dev/vda"}],
}
...
class TestProtectedInstance(base.TestCase):
    ...
    def test_make_it(self):
        ...
        self.assertEqual(EXAMPLE["description"], test_replication_pair.description)
        self.assertEqual(
            EXAMPLE["replication_model"], test_replication_pair.replication_model
        )
        self.assertEqual(EXAMPLE["status"], test_replication_pair.status)
        self.assertEqual(EXAMPLE["volume_ids"], test_replication_pair.volume_ids)
        self.assertEqual(
            EXAMPLE["server_group_id"], test_replication_pair.server_group_id
        )
        self.assertEqual(
        ...
        self.assertEqual(
            EXAMPLE["replication_status"], test_replication_pair.replication_status
        )
        self.assertEqual(EXAMPLE["fault_level"], test_replication_pair.fault_level)
        self.assertEqual(
            EXAMPLE["failure_detail"], test_replication_pair.failure_detail
        )
        self.assertEqual(EXAMPLE["created_at"], test_replication_pair.created_at)
        self.assertEqual(EXAMPLE["updated_at"], test_replication_pair.updated_at)
        self.assertEqual(
