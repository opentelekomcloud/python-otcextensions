...
EXAMPLE = {
    ...
    "name": "ipgroup-name",
    "ip_list": [{"ip": "192.168.0.2", "description": "ip description"}],
...
class TestIpAddressGroup(base.TestCase):
    ...
    def test_make_it(self):
        ...
        self.assertEqual(EXAMPLE["ip_list"][0]["ip"], sot.ip_list[0]["ip"])
        self.assertEqual(
    ...
    def test_get(self):
        ...
        mock_response.json.return_value = {"ipgroup": EXAMPLE.copy()}
    ...
    def test_update_ip_addresses(self):
        ...
        sot.update_ip_addresses(
            self.sess, ip_list=[{"ip": "192.168.0.2", "description": "test"}]
        )

        self.sess.post.assert_called_once_with(
            "elb/ipgroups/%s/iplist/create-or-update" % EXAMPLE["id"],
            json={
                "ipgroup": {"ip_list": [{"ip": "192.168.0.2", "description": "test"}]}
            },
        )
