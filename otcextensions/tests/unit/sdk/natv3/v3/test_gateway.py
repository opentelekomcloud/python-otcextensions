...
class TestGateway(base.TestCase):
    ...
    def test_basic(self):
        sot = _gateway.Gateway()
        self.assertEqual('gateways', sot.resources_name)
        self.assertEqual('gateway', sot.resource_name)
        self.assertEqual('/private-nat/gateways', sot.base_path)
        self.assertTrue(sot.allow_list)

    def test_make_it(self):
        sot = _gateway.Gateway(**EXAMPLE)
        ...
        self.assertEqual(
            EXAMPLE['downlink_vpcs'][0]['vpc_id'],
            sot.downlink_vpcs[0].vpc_id)
        self.assertEqual(
            EXAMPLE['downlink_vpcs'][0]['virsubnet_id'],
            sot.downlink_vpcs[0].virsubnet_id)
        self.assertEqual(
            EXAMPLE['downlink_vpcs'][0]['ngport_ip_address'],
