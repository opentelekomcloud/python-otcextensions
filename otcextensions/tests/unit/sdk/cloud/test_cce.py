...
class TestCceMixin(base.TestCase):
    ...
    def test_create_cce_cluster(self):
        ...
        self.register_uris(
            [
                ...
                dict(
                    method="POST",
                    uri=self.get_cce_url(base_url_append="clusters"),
                    status_code=200,
                    json={"metadata": {"uid": "123"}, "status": {"jobID": "job123"}},
                    validate=dict(
                        json={
                            "apiVersion": "v3",
                            "kind": "Cluster",
                ...
                dict(
                    method="GET",
                    uri=self.get_cce_url(base_url_append="jobs", append=["job123"]),
                    json={
                        "metadata": {"uid": "job123"},
                ...
                dict(
                    method="GET",
                    uri=self.get_cce_url(base_url_append="clusters/123"),
    ...
    def test_create_cce_cluster_bad_router(self):
        ...
        self.assertRaises(
            exceptions.SDKException, self.cloud.create_cce_cluster, **attrs
        )
    ...
    def test_create_cce_delete_cluster(self):
        ...
        self.register_uris(
            [
                dict(
                    method="GET",
                    uri=self.get_cce_url(base_url_append="clusters/cluster_name"),
                    status_code=404,
                ),
                dict(
                    method="GET",
                    uri=self.get_cce_url(base_url_append="clusters"),
                    json={
                        "items": [{"metadata": {"uid": "123", "name": "cluster_name"}}]
                    },
                ),
                dict(
                    method="DELETE",
