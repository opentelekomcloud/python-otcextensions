...
class TestVPCBandwidth(TestVpcProxy):
    ...
    def test_assign_bandwidth(self):
        self.verify_create(
            self.proxy.assign_bandwidth,
            _bandwidth.Bandwidth,
            method_kwargs={"name": "name"},
            expected_kwargs={"name": "name", "project_id": self.proxy.get_project_id()},
        )

    def test_bandwidth_delete(self):
        self.verify_delete(
            self.proxy.delete_bandwidth,
            _bandwidth.Bandwidth,
            True,
...
class TestVpc(TestVpcProxy):
    def test_vpc_create(self):
        self.verify_create(
            self.proxy.create_vpc,
            vpc.Vpc,
            method_kwargs={"name": "id"},
            expected_kwargs={"name": "id", "project_id": self.proxy.get_project_id()},
        )

    def test_vpc_delete(self):
        self.verify_delete(
            self.proxy.delete_vpc,
            vpc.Vpc,
            True,
            mock_method="otcextensions.sdk.vpc.v1._proxy.Proxy._delete",
            expected_kwargs={
...
class TestExtractName(TestVpcProxy):
    ...
    def test_extract_name(self):
        self.assertEqual(
            ["vpcs"], self.proxy._extract_name("/v1/123/vpcs", project_id="123")
        )
