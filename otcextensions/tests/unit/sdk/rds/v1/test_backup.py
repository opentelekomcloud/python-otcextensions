...
EXAMPLE_POLICY = {"keepday": 7, "starttime": "00:00:00"}
...
class TestBackup(base.TestCase):
    ...
    def test_policy_basic(self):
        ...
        self.assertEqual("/instances/%(instance_id)s/" "backups/policy", sot.base_path)
    ...
    def test_policy_update(self):
        mock_response = mock.Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"policy": copy.deepcopy(EXAMPLE_POLICY)}
        mock_response.headers = {}
        ...
        url = "/instances/%(instance_id)s/backups/policy" % {"instance_id": instance_id}

        self.sess.put.assert_called_once_with(
            url, headers=RDS_HEADERS, json={"policy": EXAMPLE_POLICY}
        )
    ...
    def test_policy_get(self):
        ...
        self.sess.get.assert_called_once_with(url, headers=RDS_HEADERS)
