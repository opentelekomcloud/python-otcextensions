...
EXAMPLE = {
    ...
    "metric": {
        ...
        "dimensions": [
            {"name": "instance_id", "value": "ccb27344-1ec4-423e-a4d9-f3a885a23e72"}
        ],
    },
    "condition": {
        "period": 1,
...
class TestAlarm(base.TestCase):
    ...
    def test_make_it(self):
        ...
        self.assertEqual(EXAMPLE["alarm_action_enabled"], sot.alarm_action_enabled)
        self.assertEqual(EXAMPLE["alarm_description"], sot.alarm_description)
        self.assertEqual(EXAMPLE["alarm_enabled"], sot.alarm_enabled)
        self.assertEqual(EXAMPLE["metric"]["namespace"], sot.metric.namespace)
        self.assertEqual(EXAMPLE["metric"]["metric_name"], sot.metric.metric_name)
        self.assertEqual(
            EXAMPLE["metric"]["dimensions"][0]["name"], sot.metric.dimensions[0].name
        )
        self.assertEqual(
            EXAMPLE["metric"]["dimensions"][0]["value"], sot.metric.dimensions[0].value
        )
        self.assertEqual(EXAMPLE["condition"]["period"], sot.condition.period)
        self.assertEqual(EXAMPLE["condition"]["filter"], sot.condition.filter)
        self.assertEqual(
            EXAMPLE["condition"]["comparison_operator"],
