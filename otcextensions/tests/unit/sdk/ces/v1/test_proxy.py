...
class TestExtractName(TestCesProxy):
    ...
    def test_extract_name(self):
        ...
        self.assertEqual(["discovery"], self.proxy._extract_name("/", project_id="123"))
        self.assertEqual(
            ["discovery"], self.proxy._extract_name("/V1.0/", project_id="123")
        )
        self.assertEqual(
            ["alarms"], self.proxy._extract_name("/V1.0/123/alarms", project_id="123")
        )
        self.assertEqual(
            ["alarm"],
            self.proxy._extract_name("/V1.0/123/alarms/some_id", project_id="123"),
        )
        self.assertEqual(
            ["alarm", "action"],
        ...
        self.assertEqual(
            ["metrics"], self.proxy._extract_name("/V1.0/123/metrics", project_id="123")
        )
        self.assertEqual(
            ["metric-data"],
            self.proxy._extract_name("/V1.0/123/metric-data?a=b", project_id="123"),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ["event-data"],
            self.proxy._extract_name("/V1.0/123/event-data", project_id="123"),
        )
        self.assertEqual(
            ["quotas"], self.proxy._extract_name("/V1.0/123/quotas", project_id="123")
        )
