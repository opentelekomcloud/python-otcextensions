...
class TestCluster(base.TestCase):
    ...
    def test_make_it(self):
        ...
        for key, value in EXAMPLE.items():
            if key in updated_sot_attrs.keys():
                ...
                self.assertEqual(sot.elb_whitelist.whitelist, value["whiteList"])
            else:
                assert_attributes_equal(self, getattr(sot, key), value)

        for i in range(len(sot.nodes)):
            instance = EXAMPLE["instances"][i]
            self.assertEqual(instance["azCode"], sot.nodes[i].availability_zone)
            ...
            assert_attributes_equal(self, sot.nodes[i].volume, instance["volume"])
    ...
    def test_create_sot(self):
        ...
        for key, value in EXAMPLE_CREATE.items():
            if key in updated_sot_attrs.keys():
                ...
                self.assertEqual(sot.disk_encryption.system_cmkid, value["systemCmkid"])
            elif key == "instance":
                self.assertEqual(sot.instance.nics.network_id, value["nics"]["netId"])
                self.assertEqual(sot.instance.nics.router_id, value["nics"]["vpcId"])
                self.assertEqual(
                ...
                self.assertEqual(sot.backup_strategy.base_path, value.pop("basePath"))
    ...
    def test_update_kernel(self):
        ...
        sot._action.assert_called_with(self.sess, "inst-type/all/image/upgrade", body)
        self.assertIsNone(rt1)
    ...
    def test_scale_in_by_node_type(self):
        ...
        self.sess.post.assert_called_with(
            f"{updated_endpoint}/clusters/{CLUSTER_ID}/role/shrink", json=json_body
        )
    ...
    def test_add_nodes(self):
        ...
        rt = sot.add_nodes(self.sess, node_type, flavor, node_size, volume_type)
        sot._action.assert_called_with(self.sess, f"type/{node_type}/independent", body)
        self.assertIsNone(rt)
