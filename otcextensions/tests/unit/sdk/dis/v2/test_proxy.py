...
class TestStream(TestDisProxy):
    ...
    def test_stream_delete(self):
        self.verify_delete(self.proxy.delete_stream, stream.Stream, True)
...
class TestCheckpoint(TestDisProxy):
    def test_checkpoint_create(self):
        self.verify_create(
            self.proxy.create_checkpoint,
            checkpoint.Checkpoint,
            method_kwargs={"arg1": "a1"},
            expected_kwargs={"arg1": "a1", "checkpoint_type": "LAST_READ"},
        )
...
class TestDumpTask(TestDisProxy):
    def test_dump_task_create(self):
        self.verify_create(
            self.proxy.create_dump_task,
            dump_task.DumpTask,
            method_kwargs={"stream_name": "test-stream", "arg1": "test-arg"},
            expected_kwargs={
                "arg1": "test-arg",
    ...
    def test_dump_task_delete(self):
        self.verify_delete(
            self.proxy.delete_dump_task,
            dump_task.DumpTask,
            False,
            method_args=["test-stream", "test-dump-task"],
            expected_kwargs={"uri_stream_name": "test-stream", "ignore_missing": False},
            expected_args=["test-dump-task"],
        )

    def test_dump_task_get(self):
        self.verify_get(
            self.proxy.get_dump_task,
            dump_task.DumpTask,
            method_args=["test-stream", "test-dump-task"],
