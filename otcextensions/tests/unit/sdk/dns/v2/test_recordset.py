...
DATA = {
    "links": {"self": "https://example.com/v2/zones/2/recordsets"},
    "recordsets": [
        {"id": "1", "name": "prod.otc.1."},
        {"id": "a3", "name": "prod.otc.2."},
        {"id": "a7", "name": "prod.otc.3."},
    ],
    "metadata": {"total_count": 7},
}
...
class TestRecordSet(base.TestCase):
    ...
    def test_get_next_link3(self):
        ...
        result = recordset.Recordset._get_next_link(
            uri=uri, response=None, data=DATA, marker=marker, limit=3, total_yielded=3
        )
        self.assertEqual(uri, result[0])
