...
EXAMPLE_CREATE = {
    ...
    "spec": {
        "flavor": "s1.large",
        "az": "eu-de",
        "login": {"sshKey": "KeyPair-demo"},
        "rootVolume": {"size": 40, "volumetype": "SATA"},
        "dataVolumes": [{"size": 100, "volumetype": "SATA"}],
        "publicIP": {
            "count": 1,
            "eip": {
                "type": "5_bgp",
                "bandwidth": {"chargemode": "traffic", "size": 10, "sharetype": "PER"},
            },
        },
        "count": 1,
    },
...
class TestClusterNode(base.TestCase):
    ...
    def test_basic(self):
        ...
        self.assertEqual("/clusters/%(cluster_id)s/nodes", sot.base_path)
    ...
    def test_list(self):
        ...
        expected_list = [
            cluster_node.ClusterNode.existing(**EXAMPLE_LIST["items"][0]),
        ]
