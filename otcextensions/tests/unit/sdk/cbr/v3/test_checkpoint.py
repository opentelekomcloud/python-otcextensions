...
class TestCheckpoint(base.TestCase):
    ...
    def test_basic(self):
        ...
        self.assertEqual("/checkpoints", sot.base_path)
    ...
    def test_make_it(self):
        ...
        self.assertEqual(EXAMPLE["status"], test_checkpoint.status)
        self.assertEqual(EXAMPLE["created_at"], test_checkpoint.created_at)
        self.assertEqual(EXAMPLE["project_id"], test_checkpoint.project_id)
        self.assertEqual(EXAMPLE["id"], test_checkpoint.id)
        self.assertEqual(
            EXAMPLE["vault"]["skipped_resources"],
            test_checkpoint.vault.skipped_resources,
        )
        self.assertEqual(EXAMPLE["vault"]["id"], test_checkpoint.vault.id)
        self.assertEqual(
        ...
        self.assertEqual(EXAMPLE["vault"]["name"], test_checkpoint.vault.name)
        self.assertEqual(
            EXAMPLE["extra_info"]["retention_duration"],
            test_checkpoint.extra_info.retention_duration,
        )
        self.assertEqual(EXAMPLE["extra_info"]["name"], test_checkpoint.extra_info.name)
