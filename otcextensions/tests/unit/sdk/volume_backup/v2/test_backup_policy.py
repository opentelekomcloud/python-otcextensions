...
def _get_fixture(name):
    fixture = os.path.join(TESTDATA_DIR, "data_files/", name)
...
class TestBackupPolicy(base.TestCase):
    ...
    def test_make_it(self):
        ...
        self.assertEqual(EXAMPLE["policy_resource_count"], sot.policy_resource_count)
        self.assertEqual(EXAMPLE["policy_resource_count"], sot.policy_resource_count)
        ...
        self.assertEqual(sp_compare["rentention_num"], sp.rentention_num)
        self.assertEqual(sp_compare["frequency"], sp.frequency)
        self.assertEqual(sp_compare["start_time"], sp.start_time)
    ...
    def test_execute_policy(self):
        ...
        self.sess.post.assert_called_once_with("backuppolicy/XX/action", json=None)
...
class TestBackupPolicyResource(base.TestCase):
    ...
    def test_link(self):
        ...
        result = self.sot.link(self.sess, "pol_id", ["volume-id-1", "volume-id-2"])
    ...
    def test_unlink(self):
        ...
        result = self.sot.unlink(self.sess, "pol_id", ["volume-id-1", "volume-id-2"])
