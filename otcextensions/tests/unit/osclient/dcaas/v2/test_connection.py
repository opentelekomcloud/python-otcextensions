...
class TestListDirectConnection(fakes.TestDcaas):
    ...
    for s in objects:
        data.append(
            (s.id, s.name, s.port_type, s.provider, s.bandwidth, s.location, s.status)
        )
...
class TestShowDirectConnection(fakes.TestDcaas):
    ...
    def test_show_non_existing(self):
        arglist = ["unexist_direct_connection"]

        verifylist = [("direct_connection", "unexist_direct_connection")]

        parsed_args = self.check_parser(self.cmd, arglist, verifylist)

        find_mock_result = exceptions.CommandError("Resource Not Found")
        self.client.find_connection = mock.Mock(side_effect=find_mock_result)
        try:
            self.cmd.take_action(parsed_args)
        except Exception as e:
            self.assertEqual("Resource Not Found", str(e))
        self.client.find_connection.assert_called_with("unexist_direct_connection")
