...
class TestCheckpoint(fakes.TestCBR):
    ...
    def test_flatten(self):
        ...
        vault = obj.vault
        ei = obj.extra_info
        cmp_data = (obj.created_at, obj.id, obj.status, vault.id, vault.name, ei.name)
...
class TestShowCheckpoint(fakes.TestCBR):
    ...
    def test_default(self):
        ...
        # Set the response
        self.client.get_checkpoint.side_effect = [self.object]

        # Trigger the action
        columns, data = self.cmd.take_action(parsed_args)

        self.client.get_checkpoint.assert_called_once_with(checkpoint="checkpoint")
...
class TestCreateCheckpoint(fakes.TestCBR):
    ...
    def test_default(self):
        arglist = [
            ...
            "resource-2-uuid",
            "--resource-details",
            "id=uuid" " name=name type=OS::Nova::Server",
        ]
        verifylist = [
            ("vault_id", "vault_id"),
            ("auto_trigger", True),
        ...
        # Set the response
        self.client.create_checkpoint.side_effect = [self.object]
