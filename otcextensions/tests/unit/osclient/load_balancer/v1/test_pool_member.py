...
class TestListPoolMember(fakes.TestLoadBalancer):
    ...
    def test_list_default(self):
        ...
        verifylist = [("pool", "pool")]
        # Verify cm is triggereg with default parameters
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)

        # Set the response
        self.client.pool_members.side_effect = [self._objects]

        # Trigger the action
        columns, data = self.cmd.take_action(parsed_args)

        self.client.pool_members.assert_called_once_with(pool="pool")
    ...
    def test_list_filter_values(self):
        ...
        self.client.pool_members.assert_called_once_with(
            pool="pool", address="address", name="name", protocol_port=15, weight=13
        )
...
class TestShowPoolMember(fakes.TestLoadBalancer):
    ...
    def test_show_default(self):
        arglist = ["pool_id", "member"]

        verifylist = [("pool", "pool_id"), ("member", "member")]
        # Verify cm is triggereg with default parameters
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)

        # Set the response
        self.client.find_pool_member.side_effect = [self._object]

        # Trigger the action
        columns, data = self.cmd.take_action(parsed_args)

        self.client.find_pool_member.assert_called_once_with(
            name_or_id="member", pool="pool_id", ignore_missing=False
        )
...
class TestCreatePoolMember(fakes.TestLoadBalancer):
    ...
    def test_create_default(self):
        ...
        # Set the response
        self.client.create_pool_member.side_effect = [self._object]
...
class TestUpdatePoolMember(fakes.TestLoadBalancer):
    ...
    def test_update_default(self):
        arglist = ["pool_id", "member", "--disable", "--name", "name", "--weight", "13"]
        ...
        # Set the response
        self.client.update_pool_member.side_effect = [self._object]
    ...
    def test_update_zero_weight(self):
        arglist = ["pool_id", "member", "--disable", "--name", "name", "--weight", "0"]
...
class TestDeletePoolMember(fakes.TestLoadBalancer):
    ...
    def test_delete_default(self):
        ...
        self.client.delete_pool_member.assert_called_once_with(
            pool_member=self._object.id, pool="pool_id", ignore_missing=False
        )
