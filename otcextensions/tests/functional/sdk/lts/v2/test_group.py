...
class TestGroup(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        attrs = {"log_group_name": self.log_group_name, "ttl_in_days": 5}
        self.log_group = self.lts.create_group(**attrs)
        self.assertIsNotNone(self.log_group.id)

    def tearDown(self):
        super(TestGroup, self).tearDown()
        log_group = self.lts.delete_group(group=self.log_group.id, ignore_missing=False)
        self.assertIsNone(log_group)
        log_group = self.lts.delete_group(group=self.log_group.id, ignore_missing=True)
        self.assertIsNone(log_group)
    ...
    def test_update(self):
        attrs = {"ttl_in_days": 7}
        log_group = self.lts.update_group(group=self.log_group.id, **attrs)
