...
class TestInstance(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        try:
            ...
            self.instance = self.conn.dms.find_instance(alias=self.INSTANCE_NAME)
            ...
            for instance in self.instances:
                if instance.id:
                    ...
                    resource.wait_for_delete(
                        session=self.conn.dms, resource=instance, interval=2, wait=60
                    )
    ...
    def _initialize_network(self):
        ...
        interface = router.add_interface(
            self.conn.network, subnet_id=TestInstance.SUBNET_ID
        )
        ...
        cidr = self.conn.compute.put(
            endpoint + self.conn.session.get_project_id() + "/vpcs/" + self.ROUTER_ID,
            data='{"vpc": {"cidr": "192.168.0.0/16"}}',
