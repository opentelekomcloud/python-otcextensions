...
-----END CERTIFICATE-----"""
    ...
    def create_listener(
        ...
        **kwargs
        ...
        if not tags:
            attrs["tags"] = [{"key": "test", "value": "api"}]
        if TestVlb.network and TestVlb.load_balancer and not TestVlb.listener:
    ...
    def create_member(
        self, name="sdk-vlb-test-member-" + uuid_v4, protocol_port=8080, **kwargs
    ):
        attrs = {
            "protocol_port": protocol_port,
            "name": name,
        ...
        if TestVlb.pool and not TestVlb.member:
            TestVlb.member = self.client.create_member(TestVlb.pool, **attrs)
        ...
        **kwargs
        ...
        if not TestVlb.certificate:
            TestVlb.certificate = self.client.create_certificate(
                private_key=private_key, certificate=certificate, name=name, **kwargs
            )
    ...
    def create_l7rule(
        ...
        **kwargs
        ...
        if TestVlb.l7policy and not TestVlb.l7rule:
            TestVlb.l7rule = self.client.create_l7_rule(TestVlb.l7policy, **attrs)

    def create_ip_address_group(
        self, name="test_ip_group-" + uuid_v4, description="Test", **kwargs
    ):
        attrs = {
            "name": name,
            "description": description,
        ...
        if not TestVlb.ip_address_group:
            TestVlb.ip_address_group = self.client.create_ip_address_group(**attrs)

    def create_security_policy(
        self, name="test_security_policy-" + uuid_v4, description="Test", **kwargs
    ):
        attrs = {
            "name": name,
            "description": description,
        ...
        if not TestVlb.security_policy:
            TestVlb.security_policy = self.client.create_security_policy(**attrs)
