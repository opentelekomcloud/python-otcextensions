...
class TestAppCode(TestApiG):
    ...
    def setUp(self):
        ...
        TestAppCode.code = self.client.create_app_code(
            self.gateway_id, TestAppCode.app.id, **attrs
        )
    ...
    def test_generate_code(self):
        code = self.client.generate_app_code(
            gateway=TestAppCode.gateway.id, app=TestAppCode.app.id
        )
        self.assertIsNotNone(code.id)
        TestAppCode.code = code

    def test_get_code(self):
        code = self.client.get_app_code(
            gateway=TestAppCode.gateway.id,
            app=TestAppCode.app.id,
            app_code=TestAppCode.code.id,
        )
