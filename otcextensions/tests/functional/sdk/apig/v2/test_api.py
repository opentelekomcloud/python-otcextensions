...
class TestApi(TestApiG):
    ...
    def setUp(self):
        ...
        self.api = self.client.create_api(gateway=self.gateway_id, **self.attrs)
        self.environment = self.client.create_environment(
            gateway=self.gateway_id, name="testPub", remark="test publish"
        )
    ...
    def test_list_apis(self):
        a = list(self.client.apis(gateway=self.gateway_id))
        self.assertGreaterEqual(len(a), 1)

    def test_get_api(self):
        api = self.client.get_api(gateway=self.gateway_id, api=self.api.id)
    ...
    def test_update_api(self):
        ...
        updated = self.client.update_api(
            gateway=self.gateway_id, api=self.api.id, **self.attrs
        )
    ...
    def test_check_api(self):
        attrs = {"type": "name", "name": self.api.name}
        check = self.client.check_api(gateway=self.gateway_id, **attrs)
    ...
    def test_debug_api(self):
        ...
        debug = self.client.debug_api(gateway=self.gateway_id, api=self.api.id, **attrs)
    ...
    def test_list_api_versions(self):
        versions = list(
            self.client.api_versions(gateway=self.gateway_id, api=self.api.id)
        )
    ...
    def test_switch_api_versions(self):
        ...
        switch = self.client.switch_version(
            gateway=self.gateway_id, api=self.api.id, version_id=publish.version_id
        )
    ...
    def test_list_api_version_details(self):
        ...
        details = list(
            self.client.api_version_details(
                gateway=self.gateway_id, version_id=publish.version_id
            )
        )
