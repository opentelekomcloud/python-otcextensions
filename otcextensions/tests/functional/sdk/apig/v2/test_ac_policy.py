...
class TestACPolicy(TestApiG):
    ...
    def test_create_ac_policy(self):
        ...
        created = self.client.create_ac_policy(gateway=TestACPolicy.gateway_id,
                                               **attrs)
    ...
    def test_update_ac_policy(self):
        ...
        updated = self.client.update_ac_policy(gateway=TestACPolicy.gateway_id,
                                               ac_policy=TestACPolicy.acl_id,
                                               **attrs)
        self.assertEqual(updated.acl_type, check)

    def test_list_ac_policies(self):
        found = list(self.client.ac_policies(gateway=TestACPolicy.gateway_id))
        self.assertGreater(len(found), 0)

    def test_get_ac_policy(self):
        found = self.client.get_ac_policy(gateway=TestACPolicy.gateway_id,
                                          ac_policy=TestACPolicy.acl_id)
        self.assertIsNotNone(found.id)

    # def test_delete_ac_policy(self):
    #     self.client.delete_ac_policy(gateway=TestACPolicy.gateway_id,
    #                                  ac_policy=TestACPolicy.acl_id)
    #     found = self.client.ac_policies(gateway=TestACPolicy.gateway_id)
    ...
    def test_delete_ac_policies(self):
        ...
        created = self.client.create_ac_policy(
            gateway=TestACPolicy.gateway_id,
            **attrs)
        attrs = {
            'acls': [TestACPolicy.acl_id, created.id]
        }
        self.client.delete_ac_policies(gateway=TestACPolicy.gateway_id,
                                       **attrs)
        found = self.client.ac_policies(gateway=TestACPolicy.gateway_id)
        self.assertEqual(len(list(found)), 0)
