...
class TestShare(base.BaseFunctionalTest):
    ...
    def _create_network(self):
        ...
        if not TestShare.network_info:
            ...
            sec_group = self.conn.network.create_security_group(name=secgroup_name)
            sec_group_2 = self.conn.network.create_security_group(name=secgroup_name_2)
    ...
    def _destroy_network(self):
        if TestShare.network_info:
            ...
            self.conn.vpc.delete_vpc(vpc, ignore_missing=False)
            ...
            sot = self.conn.network.delete_security_group(
                secgroup_2.id, ignore_missing=False
            )
            self.assertIsNone(sot)
    ...
    def test_03_extend_capacity(self):
        share = self.conn.sfsturbo.extend_capacity(TestShare.share.id, new_size=200)
    ...
    def test_04_change_security_group(self):
        ...
        share = self.conn.sfsturbo.get_share(share=share.id)
        self.assertEqual(
            share.security_group_id, TestShare.network_info["secgroup_2"].id
        )
