...
class TestInstance(TestDds):
    ...
    def test_01_create_instance(self):
        ...
        backup_strategy = {"start_time": "23:00-00:00", "keep_days": "8"}
    ...
    def test_05_bind_eip(self):
        ...
        group_id = [item for item in instance["groups"] if item["type"] == "mongos"][0][
            "nodes"
        ][0]["id"]
        ip = [
            item for item in list(self.conn.network.ips()) if item["status"] == "DOWN"
        ][0]
        eip = self.client.bind_eip(group_id, ip["floating_ip_address"], ip["id"])
        shared_data["eip"] = eip
    ...
    def test_07_enlarge_instance(self):
        ...
        group_id = [item for item in instance["groups"] if item["type"] == "shard"][0][
            "id"
        ]
        result = self.client.enlarge_instance(
            shared_data["id"], size=60, group_id=group_id
        )
    ...
    def test_12_change_security_group(self):
        ...
        instance = self.client.change_instance_security_group(
            shared_data["id"], security_group_id=sec_id
        )
    ...
    def test_13_change_ip(self):
        ...
        attrs = {"node_id": group_id, "new_ip": "192.168.0.133"}
        instance = self.client.change_instance_private_ip(shared_data["id"], **attrs)
    ...
    def test_14_create_ip(self):
        print("Creating ip")
        attrs = {"dds_type": "shard", "password": "New_pass23@"}
        print("Check for the state")
        instance = self.client.wait_normal_instance(shared_data["id"])
        print(instance["status"])
    ...
    def test_15_replica(self):
        ...
        instance = self.client.configure_client_network(
            shared_data["id"], network_ranges=["192.168.0.0/16"]
        )
        self.assertIsNotNone(instance["job_id"])
        self.client.wait_job(instance["job_id"])
