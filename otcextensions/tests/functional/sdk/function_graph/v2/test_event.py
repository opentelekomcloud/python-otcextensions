...
class TestFunctionEvent(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        self.event_attrs = {"name": "event-xx", "content": "eyJrIjoidiJ9"}
        ...
        self.addCleanup(self.conn.functiongraph.delete_event, self.function, self.event)

    def test_function_events(self):
        elist = list(self.conn.functiongraph.events(func_urn=self.function.func_urn))
        self.assertIn(self.ID, elist[0].id)

    def test_get_function_event(self):
        e = self.conn.functiongraph.get_event(self.function, self.event)
        self.assertIn(self.ID, e.id)

    def test_update_function_event(self):
        event_attrs = {"content": "ewogICAgImJvZHkiOiAiIiwKICAgICJy"}
