...
class ObsObjectTests(base.TestCase):
    ...
    def tearDown(self):
        try:
            self.openstack(
                ...
                % {"name": self.OBJECT_NAME, "container": self.CONTAINER_NAME}
            )
            self.openstack(CONTAINER_DELETE_COMMAND % {"name": self.CONTAINER_NAME})
    ...
    def setUp(self):
        ...
        json_output = json.loads(
            self.openstack(CONTAINER_CREATE_COMMAND % {"name": self.CONTAINER_NAME})
        )
        ...
        ver_fixture = fixtures.EnvironmentVariable("OS_OBS_API_VERSION", "1")
    ...
    def test_object_list(self):
        ...
        self.assertIn(self.OBJECT_NAME, [cont["name"] for cont in json_output])
