...
class TestDnat(common.NatTestCase):
    ...
    def test_nat_dnat_rule_create(self):
        ...
        json_output = json.loads(
            self.openstack("nat dnat rule list -f json " "--id " + dnat_rule_id)
        )
        self.assertIsNotNone(json_output)
        ...
        json_output = json.loads(
            self.openstack("nat dnat rule list -f json " "--nat-gateway-id " + nat_id)
        )
        self.assertIsNotNone(json_output)
        self.assertEqual(next(iter(json_output))["Nat Gateway Id"], nat_id)
        ...
        json_output = json.loads(
            self.openstack("nat dnat rule show " " -f json " + dnat_rule_id)
        )
        self.assertIsNotNone(json_output)
