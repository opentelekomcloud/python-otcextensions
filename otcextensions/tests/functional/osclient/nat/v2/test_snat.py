...
class TestSnat(common.NatTestCase):
    ...
    def test_snat_rule_list(self):
        json_output = json.loads(self.openstack("nat snat rule list -f json "))
    ...
    def test_nat_snat_rule(self):
        ...
        json_output = json.loads(
            self.openstack("nat snat rule list -f json " "--id " + snat_rule_id)
        )
        self.assertIsNotNone(json_output)
        self.assertEqual(next(iter(json_output))["Id"], snat_rule_id)
        self.assertEqual(next(iter(json_output))["Nat Gateway Id"], nat_id)

        # List Snat Rule by nat-gateway-id filter
        json_output = json.loads(
            self.openstack("nat snat rule list -f json " "--nat-gateway-id " + nat_id)
        )
        self.assertIsNotNone(json_output)
        self.assertEqual(next(iter(json_output))["Nat Gateway Id"], nat_id)
        ...
        json_output = json.loads(
            self.openstack("nat snat rule show " " -f json " + self.SNAT_RULE_ID)
        )
        self.assertIsNotNone(json_output)
