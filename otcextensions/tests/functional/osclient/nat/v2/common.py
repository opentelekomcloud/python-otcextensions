...
class NatTestCase(base.TestCase):
    ...
    def delete_snat_rule(self):
        ...
        self.openstack("nat snat rule delete " + self.SNAT_RULE_ID)
    ...
    def delete_dnat_rule(self):
        self.addCleanup(self.delete_nat_gateway)
        self.openstack("nat dnat rule delete " + self.DNAT_RULE_ID)

    def _initialize_network(self):
        router = json.loads(self.openstack("router create -f json " + self.ROUTER_NAME))
        net = json.loads(self.openstack("network create -f json " + self.NETWORK_NAME))
        self.openstack(
            "subnet create {subnet} -f json "
            "--network {net} "
            "--subnet-range 192.168.0.0/24 ".format(
                subnet=self.SUBNET_NAME, net=self.NETWORK_NAME
            )
        )
        ...
        floating_ip = json.loads(
            self.openstack(
                "floating ip create -f json "
                "{network}".format(network="admin_external_net")
            )
        )
    ...
    def _denitialize_network(self):
        self.openstack("floating ip delete " + self.FLOATING_IP_ID)
        self.openstack(
