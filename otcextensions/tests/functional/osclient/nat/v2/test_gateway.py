...
class TestGateway(common.NatTestCase):
    ...
    def test_nat_gateway_list(self):
        json_output = json.loads(self.openstack("nat gateway list -f json "))
    ...
    def test_nat_gateway(self):
        ...
        json_output = json.loads(
            self.openstack("nat gateway list -f json" " --id {}".format(nat_id))
        )
        ...
        json_output = json.loads(
            self.openstack("nat gateway list -f json" " --name {}".format(nat_name))
        )
        self.assertEqual(json_output[0]["Name"], nat_name)
        self.assertEqual(json_output[0]["Id"], nat_id)
        ...
        json_output = json.loads(
            self.openstack(
                "nat gateway list -f json" " --router-id {}".format(router_id)
            )
        )
        ...
        json_output = json.loads(self.openstack("nat gateway show -f json " + nat_name))
        self.assertEqual(json_output["name"], nat_name)
        self.assertEqual(json_output["id"], nat_id)

        # Show Nat Gateway by Id
        json_output = json.loads(self.openstack("nat gateway show -f json " + nat_id))
        self.assertEqual(json_output["name"], nat_name)
        self.assertEqual(json_output["id"], nat_id)
        ...
        json_output = json.loads(
            self.openstack(
                ...
                "--name {name} "
                '--description "{desc}" '
                "-f json".format(nat_id=nat_id, name=nat_name, desc=description)
            )
        )
        self.assertEqual(json_output["name"], nat_name)
