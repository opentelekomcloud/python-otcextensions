...
class TestRdsInstance(base.TestCase):
    ...
    def test_01_instance_list(self):
        self.openstack("rds instance list -f json ")
    ...
    def test_03_create_instance(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            ...
            # Create HA Instance
            executor.submit(self._create_instance, self.RDS_HA_NAME, "semisync")
        for instance in self.INSTANCE_LIST:
            json_output = json.loads(
                self.openstack("rds instance show -f json " + instance)
            )
    ...
    def test_05_backup_list(self):
        instance_name = self.INSTANCE_LIST[0]
        json_output = json.loads(
            self.openstack("rds backup list -f json " + instance_name)
        )
        self.assertIsNotNone(json_output)
        ...
        json_output = json.loads(
            self.openstack(
                ...
                "{name} {instance} --wait --wait-interval 10".format(
                    name=self.BACKUP_NAME, instance=instance_name
                )
            )
        )
        self.assertIsNotNone(json_output)
    ...
    def test_07_backup_download_links(self):
        ...
        for backup in json_output:
            json_output = json.loads(
                self.openstack("rds backup download links -f json " + backup["ID"])
            )
            self.assertIsNotNone(json_output)
    ...
    def test_09_backup_delete(self):
        ...
        self.assertRaises(
            CommandFailed, self.openstack, "rds backup delete " + self.BACKUP_ID
        )
    ...
    def test_10_backup_policy_set(self):
        ...
        json_output = json.loads(
            self.openstack("rds instance backup policy show -f json " + instance_name)
        )
    ...
    def test_12_delete_instance(self):
        ...
        for instance in self.INSTANCE_LIST:
            self.assertRaises(
                CommandFailed, self.openstack, "rds instance show " + instance
            )
        time.sleep(60)

    def _create_instance(
        self, instance_name, ha_mode=None, backup=None, from_instance=None
    ):
        cli_str = (
            "rds instance create {name}"
        ...
        if ha_mode:
            cli_str += (
                ...
                " --ha-mode {ha_mode}".format(
                    flavor=self.HA_FLAVOR, az=self.HA_AZ, ha_mode=ha_mode
                )
            )
        else:
        ...
        if backup or from_instance:
            cli_str += " --from-instance {instance}" " --backup {backup}".format(
                instance=from_instance, backup=backup
            )
    ...
    def _get_region(self):
        json_output = json.loads(self.openstack("region list -f json"))
        return json_output[0]["Region"]

    def _get_latest_version(self):
        json_output = json.loads(
            self.openstack("rds datastore version list -f json " + self.DATASTORE)
        )
    ...
    def _get_default_flavor(self, flavor_type=None):
        ...
        json_output = json.loads(
            self.openstack(
                "rds flavor list -f json {} {}".format(self.DATASTORE, self.VERSION)
            )
        )
        if flavor_type:
            flavor_list = [
                [data["vcpus"], data["name"]]
                for data in json_output
                if data["instance_mode"] == flavor_type.lower()
            ]
        else:
            flavor_list = [
                [data["vcpus"], data["name"]]
                for data in json_output
                if data["instance_mode"] == "single"
            ]
        flavor_list = sorted(flavor_list, key=lambda x: int(x[0]))
        return flavor_list[0][1]

    def _delete_instance(self, instance_name):
        self.openstack("rds instance delete --wait " + instance_name)
    ...
    def _initialize_network(self):
        ...
        self.openstack(
            ...
            "--subnet-range 192.168.0.0/24 ".format(
                subnet=self.SUBNET_NAME, net=self.NET_NAME
            )
        )
        sg = json.loads(self.openstack("security group create -f json " + self.SG_NAME))

        self.openstack(
            "router add subnet {router} "
            "{subnet} ".format(router=self.ROUTER_NAME, subnet=self.SUBNET_NAME)
        )
    ...
    def _deinitialize_network(self):
        ...
        self.openstack("security group delete " + self.SG_NAME)
