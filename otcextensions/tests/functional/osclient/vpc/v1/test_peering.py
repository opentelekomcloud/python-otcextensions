...
class TestPeering(common.VpcTestCase):
    ...
    def test_vpc_peering_list(self):
        json_output = json.loads(self.openstack("vpc peering list -f json "))
    ...
    def test_vpc_peering(self):
        ...
        json_output = json.loads(
            self.openstack("vpc peering list -f json " "--id {}".format(peering_id))
        )
        ...
        json_output = json.loads(
            self.openstack("vpc peering list -f json " "--name {}".format(peering_name))
        )
        self.assertEqual(json_output[0]["Name"], peering_name)
        self.assertEqual(json_output[0]["Id"], peering_id)
        ...
        json_output = json.loads(
            self.openstack(
                "vpc peering list -f json " "--router-id {}".format(local_router_id)
            )
        )
        self.assertIsNotNone(json_output)
        ...
        json_output = json.loads(
            self.openstack(
                "vpc peering list -f json " "--router-id {}".format(peer_router_id)
            )
        )
        for peering in json_output:
            self.assertEqual(peering["Peer Router Id"], peer_router_id)

        # Show Vpc Peering by Name
        json_output = json.loads(
            self.openstack("vpc peering show -f json " + peering_name)
        )
        ...
        json_output = json.loads(
            self.openstack("vpc peering show -f json " + peering_id)
        )
        self.assertEqual(json_output["name"], peering_name)
        self.assertEqual(json_output["id"], peering_id)
        ...
        json_output = json.loads(
            self.openstack(
                ...
                '--description "{desc}" '
                "-f json".format(
                    peering_id=peering_id, name=peering_name, desc=description
                )
            )
        )
        self.assertEqual(json_output["name"], peering_name)
