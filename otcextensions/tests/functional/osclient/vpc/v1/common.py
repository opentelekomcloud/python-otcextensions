...
class VpcTestCase(base.TestCase):
    ...
    def _create_routers(self):
        local_router = json.loads(
            self.openstack("router create -f json " + self.LOCAL_ROUTER_NAME)
        )
        self.LOCAL_ROUTER_ID = local_router["id"]

        peer_router = json.loads(
            self.openstack("router create -f json " + self.PEER_ROUTER_NAME)
        )
        self.PEER_ROUTER_ID = peer_router["id"]

    def _delete_routers(self):
        self.openstack(
            "router delete {} {}".format(self.LOCAL_ROUTER_ID, self.PEER_ROUTER_ID)
        )
